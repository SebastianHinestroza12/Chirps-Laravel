            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form action={{route('chirps.store')}} method="POST">
                        ...
                        <textarea placeholder="{{__('What are you thinking')}}" class="mb-1 bg-transparent block w-full rounded-md border-gray-300" name="message" id="">{{old('message')}}</textarea>
                        {{-- @error('message')
                            {{$message}}
                        @enderror --}}
                        <x-input-error :messages="$errors->get('message')"/>
                        <x-primary-button class="mt-4">{{__('Crear')}}</x-primary-button>
                    </form>
                </div>
            </div>
        </div>
